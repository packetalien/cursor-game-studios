...
Templates are in `.cursor/docs/templates/`:
...
Also in `.cursor/docs/templates/collaborative-protocols/` (used by agents, not typically edited directly):
...
2. **Set up the engine** — Run `/setup-engine` (uses the brainstorm recommendation)
   ...
   - Creates `.cursor/docs/technical-preferences.md` with naming conventions,
