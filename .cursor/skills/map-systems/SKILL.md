...
**After dependency mapping is approved, spawn `technical-director` via Task using gate TD-SYSTEM-BOUNDARY (`.cursor/docs/director-gates.md`) before proceeding to priority assignment.**
...
**After priorities are approved, spawn `producer` via Task using gate PR-SCOPE (`.cursor/docs/director-gates.md`) before writing the index.**
...
Using the template at `.cursor/docs/templates/systems-index.md`, populate the
...
**After the systems index is written, spawn `creative-director` via Task using gate CD-SYSTEMS (`.cursor/docs/director-gates.md`).**
...
### Rich procedural spine (`map-systems`)
...
```text
You are executing the Cursor Game Studios skill `map-systems`.
