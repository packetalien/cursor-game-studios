...
After completing the deviation checks in Phase 4, spawn `qa-lead` via Task using gate **QL-TEST-COVERAGE** (`.cursor/docs/director-gates.md`).
...
Spawn `lead-programmer` via Task using gate **LP-CODE-REVIEW** (`.cursor/docs/director-gates.md`).
...
### Rich procedural spine (`story-done`)
...
```text
You are executing the Cursor Game Studios skill `story-done`.
