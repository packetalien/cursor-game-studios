...
After all epics for the current layer are defined (Step 4 completed for all in-scope systems), and before writing any files, spawn `producer` via Task using gate **PR-EPIC** (`.cursor/docs/director-gates.md`).
...
### Rich procedural spine (`create-epics`)
...
```text
You are executing the Cursor Game Studios skill `create-epics`.
