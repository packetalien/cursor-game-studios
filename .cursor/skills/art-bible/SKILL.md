...
Read `.cursor/docs/technical-preferences.md` if it exists — extract performance budgets and engine for asset standard constraints.
...
After all sections are complete (or the scoped set from Phase 1 is complete), spawn `creative-director` via Task using gate **AD-ART-BIBLE** (`.cursor/docs/director-gates.md`).
...
- Does `.cursor/docs/technical-preferences.md` contain a configured engine (not `[TO BE CONFIGURED]`)? → setup-engine is done, skip that option
...
### Rich procedural spine (`art-bible`)
...
```text
You are executing the Cursor Game Studios skill `art-bible`.
